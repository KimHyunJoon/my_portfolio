...
function modal_resize_close(e) {
  if (window.innerWidth > 1022) {
    modal_bg.style.display = "none";
  }
}
function close_exp(e) {
  if (window.innerHeight < 1022) {
    for (let i = 0; i < explain_content.length; i++) {
      explain_content[i].classList.remove("active");
    }
    for (let i = 0; i < clk_circle.length; i++) {
      clk_circle[i].classList.remove("active");
    }
  } 
}
close_modal.addEventListener("click", f_close_modal);
window.addEventListener("resize", close_exp);
window.addEventListener("resize", modal_resize_close);
